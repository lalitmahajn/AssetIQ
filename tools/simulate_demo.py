...
def simulate():
    ...
    try:
        ...
        for r_name in reasons:
            ...
            exists = (
                db.query(MasterItem)
                .filter_by(master_type_code="STOP_REASON", item_code=item_code)
                .first()
            )
            if not exists:
        ...
        # 3. Generate Historical Data (Past 30 Days + Today)
        ...
        asset_profiles = {
            "R-101": {
                ...
                "min_dur": 100,
                "max_dur": 240,
            },  # Aggressive: Will be RED (<70%)
            "COL-501": {
                "max_stops": 4,
                "min_dur": 45,
                "max_dur": 150,
            },  # Moderate: Will be YELLOW (70-90%)
            "EX-201": {
                "max_stops": 2,
                "min_dur": 5,
                "max_dur": 45,
            },  # Efficient: Will be GREEN (>90%)
            "D-401": {
                "max_stops": 2,
                "min_dur": 5,
                "max_dur": 45,
        ...
        while current_date <= today_date:
            ...
            for asset_id in asset_ids:
                profile = asset_profiles.get(
                    asset_id, {"max_stops": 2, "min_dur": 5, "max_dur": 60}
                )

                num_stops = random.randint(1, profile["max_stops"])
                for _ in range(num_stops):
                    # Random time during the day
                    if max_hour == 0 and is_today:
                        hour = 0
                        minute = random.randint(0, now.minute)
                    else:
                        hour = random.randint(0, max_hour)
                        minute = random.randint(0, 59 if hour < max_hour else now.minute)
                    ...
                    te_list = (
                        ...
                        .filter(
                            TimelineEvent.correlation_id.in_(
                                [f"stop_open:{stop_id}", f"ticket_open:{ticket_id}"]
                            )
                        )
                        .all()
                    )
                    ...
                    audit_entries = (
                        db.query(AuditLog)
                        .filter(AuditLog.entity_id.in_([stop_id, ticket_id]))
                        .all()
                    )
            ...
            for r_code, (count, d_min) in reason_map.items():
                stop_reasons.append(
                    {"reason_code": r_code, "stops": count, "downtime_minutes": d_min}
                )
                ...
                hq_tickets.append(
                    {
                        ...
                        "payload": {
                            ...
                            "sla_due_at_utc": t.sla_due_at_utc.isoformat() + "Z"
                            if t.sla_due_at_utc
                            else None,
                        },
        ...
        report_start = (today_date - timedelta(days=7)).isoformat()
        ...
        print("- 30 days of historical data + TODAY generated.")
