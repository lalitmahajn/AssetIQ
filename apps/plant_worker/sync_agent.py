...
from apps.plant_backend.models import DeadLetter, EmailQueue, EventOutbox
    ...
    try:
        rows = (
            db.execute(
                ...
                .where(
                    (EventOutbox.next_attempt_at_utc.is_(None))
                    | (EventOutbox.next_attempt_at_utc <= _now())
                )
        ...
        body = json.dumps({"items": items}, separators=(",", ":"), ensure_ascii=False).encode(
            "utf-8"
        )
        headers = {
            "Content-Type": "application/json",
            "X-Signature": _sign(body, settings.sync_hmac_secret),
            "X-Kid": settings.sync_hmac_kid,
        }
                ...
                if r.retry_count >= MAX_RETRIES:
                    db.add(
                        DeadLetter(
                            ...
                            entity_type=r.entity_type,
                            correlation_id=r.correlation_id,
                            payload_json=json.dumps(r.payload_json),
                            error=err,
                            created_at_utc=now,
                        )
                    )
                    db.add(
                        EmailQueue(
                            ...
                            subject=f"[{settings.plant_site_code}] SYNC DEAD-LETTER {r.correlation_id}",
                            body=f"Outbox moved to dead-letter. Correlation={r.correlation_id} Error={err}",
                            status="PENDING",
                            created_at_utc=now,
                            sent_at_utc=None,
                        )
