...
from apps.plant_backend.models import MasterType, MasterItem
...
def list_items(type_code: str, user=Depends(require_perm("master.view"))):
    db = PlantSessionLocal()
    try:
        rows = db.execute(select(MasterItem).where(
            MasterItem.master_type_code == type_code,
            MasterItem.is_active == True
        ).order_by(MasterItem.item_code)).scalars().all()
